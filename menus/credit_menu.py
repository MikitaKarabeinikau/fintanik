...
def get_credit_menu():
    ...
    keyboard = [
        ...
        [KeyboardButton(f"{emoji('UPDATE')} Update Credit/Loan")],
...
async def handle_credit_menu(update: Update, context: ContextTypes.DEFAULT_TYPE):
    ...
    if update.message.text == f"{emoji('NEW')} Add Credit/Loan":
        ...
        return await flow
    elif update.message.text == f"{emoji('UPDATE')} Update Credit/Loan":
        ...
        await update.message.reply_text("Feature to delete credit/loan is under development.")
    elif update.message.text == f"{emoji('STATS')} View Credit/Loan Statistics":
        await update.message.reply_text("Feature to view credit/loan statistics is under development.")
        ...
        await update.message.reply_text(f"{show_credit_menu()}",reply_markup=get_credit_menu())
...
async def receive_last_payment(update: Update, context: ContextTypes.DEFAULT_TYPE):
    ...
    try:
        ...
        create_credit(user_id=update.effective_user.id,
...
def show_credit_menu():
    return f'Displaying Info in development mode.'